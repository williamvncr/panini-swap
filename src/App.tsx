...
const SECTIONS: { prefix: string; label: string; flag: string; codes: string[] }[] = [
  { prefix:"00",  flag:"⭐", label:"Especial",         codes: ["00"] },
  { prefix:"FWC", flag:"🏆", label:"FIFA WC",         codes: Array.from({length:19},(_,i)=>`FWC${i+1}`) },
...
export default function PaniniSwap() {
  ...
  const toggleWant=useCallback((c:string)=>{
    setWant(p=>{
      const cur=p||[];
      if(cur.includes(c)) return cur.filter(x=>x!==c);
      return [...cur,c];
    });
    // Quitar de repetidas si se agrega a buscadas
    setHave(h=>(h||[]).filter(x=>x!==c));
  },[]);
        ...
        {!accessGranted && (allPlayers||[]).filter(p=>p.paid).length < 25 && (
          <div style={{background:"#14532d",borderBottom:"2px solid #4ade8044",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"12px",flexWrap:"wrap" as const}}>
            <div style={{display:"flex",alignItems:"center",gap:"10px",flex:1,minWidth:0}}>
              ...
              <div>
                <p style={{color:"#86efac",fontWeight:"700",fontSize:"13px",margin:"0 0 2px",lineHeight:"1.4"}}>
                  ¡Primeros {25-(allPlayers||[]).filter(p=>p.paid).length} cupos gratis!
                </p>
                <p style={{color:"#bbf7d0",fontSize:"12px",margin:0,lineHeight:"1.4"}}>
                  Los primeros 25 coleccionistas reciben su código sin costo. Escribinos por WhatsApp para obtener el tuyo.
                </p>
              </div>
            </div>
            <a href="https://wa.me/50660582201?text=Hola%2C%20quiero%20mi%20c%C3%B3digo%20gratis%20de%20Panini%20Swap%20%F0%9F%8E%81"
              target="_blank" rel="noopener noreferrer"
              style={{background:"#25d366",borderRadius:"8px",padding:"8px 14px",color:"#fff",fontWeight:"700",fontSize:"12px",textDecoration:"none",whiteSpace:"nowrap" as const,flexShrink:0,display:"flex",alignItems:"center",gap:"6px"}}>
